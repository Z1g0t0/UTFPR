...
# Caso offline comentar as linhas docker pull
# Offline porem nao podera finalizar pagamento
...
if [ -z "$STRIPE_API_KEY" ] || [ -z "$STRIPE_SECRET_KEY" ]; then
    echo "STRIPE_API_KEY ou STRIPE_SECRET_KEY nao configurado"
    exit 1
fi
...
env_var_handler "pagamentoPORT" port_handler
...
dockerRM stripe
STRIPE_ID=$(docker run -i -d --network="host" stripe/stripe-cli listen --forward-to http://localhost:$pagamentoPORT/stripe-webhook --api-key $STRIPE_SECRET_KEY)

sleep 4

echo "STRIPE CONTAINER ID: $STRIPE_ID"
STRIPE_WEBHOOK_SECRET=$(docker exec $STRIPE_ID stripe listen --print-secret --api-key $STRIPE_SECRET_KEY)
...
sleep 3
cd backend
uvicorn "pagamento:app" --port "$port" &
python "pagamento.py" &
cd ..
# === || ===


# Resto das REST API's
