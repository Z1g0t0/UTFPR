...
void ppos_init () 
{
    ...
    main_task.time_at_birth = systime();
    main_task.process_enter = 0;
    main_task.process_time = 0;
...
int task_create( task_t *task, void (*start_routine)(void *), void *arg)
{
	...
	task->time_at_birth = systime();
	task->time_at_death = 0;
	task->process_time = 0;
	task->activations = 0;
    }
    else
...
void task_exit (int exit_code) 
{
	...
	else
	{
	    ...
	    --userTasks;
	    current_task->time_at_death = systime();
	    int execution_time =    current_task->time_at_death - 
				    current_task->time_at_birth;

	    printf( "\nTask %d exit:  execution time %d ms, processor time %d ms,"
		    " %d activations\n\n", task_id(), execution_time, 
		    current_task->process_time, current_task->activations );
	    task_yield();
	}
    }
    else
...
int task_switch (task_t *task) 
{
    ...
    // Caso 1: Fim de programa dispatcher -> main.
    if ( task == &main_task )
    {
	if( current_task != &dispatcher_task )
	{
	...
	int execution_time =    current_task->time_at_death - 
				current_task->time_at_birth;
	printf( "\nDispatcher exit: execution time %d ms, "
		"processor time %d ms, %d activations\n\n",
		execution_time, current_task->process_time, 
		current_task->activations );

	swapcontext(&dispatcher_task.context, &main_task.context);
    }

    // Caso 2: Começo de programa main -> dispatcher.
    ...
    // Caso 3: Troca entre tarefa usuario -> dispatcher.
	...
	current_task = &dispatcher_task;
	current_task->process_enter = systime ();
	aux->process_time += systime() - aux->process_enter;
    	swapcontext(&aux->context, &current_task->context);
    }

    // Caso 4: Troca entre dispatcher -> tarefa usuario.
    else
    {
	if( current_task != &dispatcher_task )
	{
